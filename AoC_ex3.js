...
wire1.forEach(str => {
  if (str[0] == "R") {
    for (let i = 1; i <= Number(str.slice(1)); i++) {
      pathWire1.push([
        pathWire1[pathWire1.length - 1][0] + 1,
        pathWire1[pathWire1.length - 1][1]
      ]);
    }
  }
  if (str[0] == "L") {
  ...
  if (str[0] == "U") {
    for (let i = 1; i <= Number(str.slice(1)); i++) {
      pathWire1.push([
        pathWire1[pathWire1.length - 1][0],
        pathWire1[pathWire1.length - 1][1] + 1
      ]);
    }
  }
  if (str[0] == "D") {
...
wire2.forEach(str => {
  if (str[0] == "R") {
    for (let i = 1; i <= Number(str.slice(1)); i++) {
      pathWire2.push([
        pathWire2[pathWire2.length - 1][0] + 1,
        pathWire2[pathWire2.length - 1][1]
      ]);
    }
  }
  if (str[0] == "L") {
  ...
  if (str[0] == "U") {
    for (let i = 1; i <= Number(str.slice(1)); i++) {
      pathWire2.push([
        pathWire2[pathWire2.length - 1][0],
        pathWire2[pathWire2.length - 1][1] + 1
      ]);
    }
  }
  if (str[0] == "D") {
...
const result = [];
for (let i = 0; i < pathWire1.length; i++) {
  for (let j = 0; j < pathWire2.length; j++) {
    if (
      pathWire1[i][0] === pathWire2[j][0] &&
      pathWire1[i][1] === pathWire2[j][1]
    ) {
      result.push(pathWire1[i]);
    }
  }
}
const min = Math.min(...result.map(el => Math.abs(el[0]) + Math.abs(el[1])).filter(x=>x!=0));
console.log(min);
//console.log(pathWire2);
//console.log(pathWire2)
//console.log(result)